...
const auth = async (request,response,next) => {
    try{
        ...
        const decoded = jwt.verify(token,'thisismynewsignin'); //verify token extracted from Authorization header
        ...
        next(); //continue 
    }catch(e){ response.status(401).send({error:'Please authenticate.'}) }
}
